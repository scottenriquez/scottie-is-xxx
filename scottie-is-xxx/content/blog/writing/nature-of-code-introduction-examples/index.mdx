...
## Random Walk
A random walk traces a path through a Cartesian plane going in a random direction with each step (i.e., one pixel). The walks are built by plotting individual pixels as rectangles in a scalable vector graphic (SVG). The program creates a purple pixel at `(200, 400)`. The `walk` function chooses a random direction, paints another white pixel, and updates the internal state for current coordinates.
```javascript
walk() {
...
The `RandomWalk` function calls the `Walk` or `WalkWeightedRight` function until an edge is hit. The SVG is the rendered.
```javascript
...
```
Note that generate numerous random walks in rapid succession may cause the browser to crash given the number of pixels.
<div>
